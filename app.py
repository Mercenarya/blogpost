...
class Response_post(Resource):
    def get(self):
        return {'message':'api-post-complete'}
...
class Response_post_tasks(Resource):  
    def get(self):
        id = ""
        description = ""
        notification = ""
        ...
        if request.method == "POST":
            ...
            task = f"api-checker-{request.method}"
            description = f"{request.get_data}"
            header = f"{request.get_json(request.headers)}"
        if request.method == "GET":
            id = f"{2}"
            notification = "GET method"
            task = f"api-checker-{request.method}"
            description = f"{request.form.get("")}"
...
api.add_resource(Response_post_tasks,"/api")
api.add_resource(Response_role,"/role")
    ...
    try:
        ...
        if session.get("role") != "admin":
            ...
            if users and request.method == "POST":
                ...
                delcmd = "DELETE FROM users WHERE id = %s" 
                cursor.execute(delcmd,(id,))
                notifiation = f"User ID {id} deleted"
                db.commit()
                
            return render_template('dashboard.html',users=users,msg=notifiation)
